...
public class player2move : MonoBehaviour
{
    ...
    {   
        Vector3 moveDirection = Vector3.zero;
        #if ENABLE_INPUT_SYSTEM
            if (Keyboard.current.leftArrowKey.isPressed || isLeftPressed)
            {
                moveDirection += Vector3.left;
            }
            if (Keyboard.current.rightArrowKey.isPressed || isRightPressed)
            {
                moveDirection += Vector3.right;
            }
            if (Keyboard.current.upArrowKey.isPressed || isUpPressed)
            {
                moveDirection += Vector3.forward;
            }
            if (Keyboard.current.downArrowKey.isPressed || isDownPressed)
            {
            ...
            if (Input.GetKey(KeyCode.RightArrow) || isRightPressed)
            {
                moveDirection += Vector3.right;
            }
            if (Input.GetKey(KeyCode.UpArrow) || isUpPressed)
            {
                moveDirection += Vector3.forward;
            }
            if (Input.GetKey(KeyCode.DownArrow) || isDownPressed)
            {
                moveDirection += Vector3.back;
            }
        #endif
